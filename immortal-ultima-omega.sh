...
enable_service() {
  ...
  systemctl daemon-reload && systemctl restart "$svc" >> "$LOG_FILE" 2>&1 || true
  if systemctl is-active --quiet "$svc"; then plane "Recovered via restart"; return 0; fi
  planf "Plan F: unit recreation fallback"
  systemctl daemon-reload && systemctl start "$svc" >> "$LOG_FILE" 2>&1 || true
...
if [[ $DRY_RUN -eq 1 ]]; then
  ...
  if fwupdmgr get-updates --json 2>/dev/null | grep -q '"Updates"'; then
    ...
    fwupdmgr update --assume-yes --no-reboot-check >> "$LOG_FILE" 2>&1 \
      || planc "Some firmware updates deferred (safe)"
  else
...
KVM-PC = true
DESKEOF
    chown -R "$REAL_USER:$REAL_USER" "$DESKFLOW_DIR"
...
for candidate in \
    ...
    "$REAL_HOME/Bureau" \
    "$(su -c 'xdg-user-dir DESKTOP' "$REAL_USER" 2>/dev/null || echo '')"; do
